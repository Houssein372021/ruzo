            ...
            {filteredCustomers.map((customer) => (
              <tr key={customer.id}>
                <td>
                  ...
                  <div className="text-xs text-[#444444]">{customer.phone}</div>
                </td>
              ...
              <section className="border border-[#E5E5E5] bg-white">
                ...
                <div className="divide-y divide-[#E5E5E5]">
                  {(selectedCustomer.orders ?? []).length === 0 ? (
                    <p className="px-5 py-5 text-sm text-[#555555]">{t("noOrders")}</p>
                  ) : (
                    selectedCustomer.orders?.map((order) => (
                      <div key={order.id} className="grid gap-4 px-5 py-4 lg:grid-cols-[1fr_auto_auto] lg:items-start">
                        <div>
                          <p className="font-semibold">{order.orderNumber}</p>
                          <p className="mt-1 text-xs uppercase tracking-[0.14em] text-[#444444]">
...
function SummaryBox({ label, value }: { label: string; value: string }) {
  return (
    <div className="border border-[#E5E5E5] bg-white p-4">
      <p className="text-xs uppercase tracking-[0.14em] text-[#444444]">{label}</p>
