    ...
    <div className="space-y-6">
      ...
      {selectedOrder ? (
        <div className="fixed inset-0 z-50 overflow-y-auto bg-[#080808]/45 px-4 py-8">
          <div className="mx-auto w-full max-w-4xl border border-[#E5E5E5] bg-[#FFFFFF] shadow-2xl">
            <div className="flex items-start justify-between gap-6 bg-[#080808] px-6 py-6 text-[#FFFFFF] sm:px-8">
              <div>
                ...
                <p className="mt-2 text-sm text-[#F5F5F5]">{t(orderStatusTranslationKeys[selectedOrder.status])}</p>
              </div>
              <button
                type="button"
                className="grid h-11 w-11 place-items-center border border-[#F5F5F5]/45 text-[#FFFFFF] transition hover:bg-[#FFFFFF]/10"
            ...
            <div className="grid gap-7 px-6 py-7 sm:px-8">
              <div className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
                <section className="border border-[#E5E5E5] bg-white p-5">
                  <div className="mb-5 flex items-center justify-between gap-4">
                    <div>
                      <p className="text-[0.68rem] font-semibold uppercase tracking-[0.18em] text-[#444444]">
                        {t("customerInfo")}
                      </p>
                    ...
                    {whatsappUrl(selectedOrder.customerPhone) ? (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="inline-flex h-10 items-center gap-2 border border-[#080808] px-3 text-xs font-semibold uppercase tracking-[0.14em] text-[#080808] transition hover:bg-[#080808] hover:text-[#FFFFFF]"
                      >
                        <MessageCircle className="h-4 w-4" />
                        {t("whatsapp")}
                      </a>
                  ...
                  <dl className="grid gap-4 text-sm sm:grid-cols-2">
                    <div className="flex gap-3">
                      <Mail className="mt-0.5 h-4 w-4 shrink-0 text-[#444444]" />
                      <div>
                        <dt className="text-xs uppercase tracking-[0.14em] text-[#444444]">{t("email")}</dt>
                        <dd className="mt-1 wrap-break-word font-medium">{displayText(selectedOrder.email)}</dd>
                      </div>
                    </div>
                    <div className="flex gap-3">
                      <Phone className="mt-0.5 h-4 w-4 shrink-0 text-[#444444]" />
                      <div>
                        <dt className="text-xs uppercase tracking-[0.14em] text-[#444444]">{t("phone")}</dt>
                    ...
                    <div className="flex gap-3 sm:col-span-2">
                      ...
                      <div>
                        <dt className="text-xs uppercase tracking-[0.14em] text-[#444444]">{t("address")}</dt>
                ...
                <section className="border border-[#E5E5E5] bg-white p-5">
                  ...
                  <dl className="mt-5 grid gap-4 text-sm">
                    <div className="flex gap-3">
                      <CalendarDays className="mt-0.5 h-4 w-4 shrink-0 text-[#444444]" />
                      <div>
                        <dt className="text-xs uppercase tracking-[0.14em] text-[#444444]">Date</dt>
                    ...
                    <div className="flex justify-between gap-4 border-t border-[#E5E5E5] pt-4">
                      <dt className="text-xs uppercase tracking-[0.14em] text-[#444444]">Payment</dt>
                      <dd className="text-right font-medium">{displayText(selectedOrder.paymentMethod)}</dd>
                    </div>
                    <div className="flex justify-between gap-4 border-t border-[#E5E5E5] pt-4">
                      <dt className="text-xs uppercase tracking-[0.14em] text-[#444444]">{t("whatsapp")}</dt>
                    ...
                    {selectedOrder.reviewToken ? (
                      <div className="border-t border-[#E5E5E5] pt-4">
                        <dt className="text-xs uppercase tracking-[0.14em] text-[#444444]">{t("reviewLink")}</dt>
                        <dd className="mt-2 grid gap-2">
                          ...
                          <button
                            type="button"
                            className="inline-flex w-fit items-center gap-2 border border-[#080808] px-3 py-2 text-xs font-semibold uppercase tracking-[0.14em] text-[#080808] transition hover:bg-[#080808] hover:text-[#FFFFFF]"
                          ...
                          {selectedOrder.status !== "DELIVERED" ? (
                            <p className="text-xs leading-5 text-[#444444]">{t("reviewLinkAfterDeliveryAdmin")}</p>
                          ) : null}
                          <p className="text-xs leading-5 text-[#444444]">
              ...
              <section className="border border-[#E5E5E5] bg-white">
                <div className="flex items-center justify-between border-b border-[#E5E5E5] px-5 py-4">
                  ...
                  <span className="text-xs font-semibold uppercase tracking-[0.16em] text-[#444444]">
                  ...
                  {(selectedOrder.items ?? []).map((item) => (
                    <div key={item.id} className="grid gap-4 px-5 py-4 sm:grid-cols-[1fr_auto_auto] sm:items-center">
                      ...
                      <div className="text-sm sm:text-center">
                        <p className="text-xs uppercase tracking-[0.14em] text-[#444444]">{t("quantity")}</p>
                        <p className="mt-1 font-semibold">{item.quantity}</p>
                      </div>
                      <div className="text-sm sm:min-w-28 sm:text-right">
                        <p className="text-xs uppercase tracking-[0.14em] text-[#444444]">{t("total")}</p>
              ...
              <div className="grid gap-5 lg:grid-cols-[1fr_20rem]">
                ...
                <section className="border border-[#E5E5E5] bg-white p-5">
                  <dl className="grid gap-3 text-sm">
                    ...
                    <div className="flex justify-between gap-4 border-t border-[#E5E5E5] pt-4 text-lg font-semibold">
                      <dt>{t("total")}</dt>
