  ...
  return (
    <>
      ...
      <section className="mx-auto max-w-7xl px-5 py-16 lg:px-10">
        <p className="text-xs font-semibold uppercase tracking-display text-[#080808]">{t("contact")}</p>
        ...
        <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {contactItems.map((item) => {
            ...
            const content = (
              <>
                <Icon className="h-6 w-6 text-[#080808]" />
                <p className="font-display mt-7 text-xl text-[#080808]">{item.label}</p>
                <p className="mt-3 text-sm text-[#080808]">{item.value}</p>
              </>
            );
            ...
            if (item.href) {
              return (
                <a
                  ...
                  className="block border border-[#080808]/10 bg-white p-7 transition hover:-translate-y-1 hover:border-[#080808]"
                >
                  {content}
                </a>
              );
